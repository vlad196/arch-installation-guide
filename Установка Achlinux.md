...
**1) Если решили заново настроить и нет уже готовых ключей для подписи, то убеждаемся, что Secure-boot в прошивке стоит на  None ( Это означает, что Secure-boot сброшен, ключи пусты и устройство готово к внедрению новых ключей)**

**2) Проверяем, что TPM включён**

**3) В Boot entry, USB переносил в самый вверх списка**
...
>[!Note]
>Подключение к wifi через iwctl, он работает интерактивно.
...
device list
```
**Запускаем сканирование сети (команда ничего не выведет):**
```bash
station _устройство_ scan
```
**После этого выводим список обнаруженных сетей:**
```bash
station _устройство_ get-networks
```
**Подключаемся к сети сразу с паролем:**
```bash
iwctl --passphrase _пароль_ station _устройство_ connect _SSID_
```
...
```bash
sudo -u vlad paru -S --needed --asdeps wireless-regdb linux-firmware  modprobed-db
```
